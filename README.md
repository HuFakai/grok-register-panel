...
| `proxy_fallback_to_local` | 面板代理池无健康且启用代理时回退本地网络：`true` 用 `proxy`（留空则浏览器直连），默认 `false` |
| `proxies.txt` | 可选的旧版多行代理文件；未配置面板代理池时继续兼容 |
| `register_workers` | 并发浏览器数（建议先 2～3） |
| `register_count` | 单次目标数量 |
| `accounts_per_ip` | 每个真实出口最多完成的账号槽位；`0`=不限（只按连续失败换 IP）；成功和最终失败都计数 |
| `ip_failure_rotate_threshold` | 连续最终失败多少次后换 IP（始终生效），默认 `3` |
| `sso_wait_timeout` | SSO 总等待窗口，默认 `50` 秒，范围 `15-180` |
| `sso_poll_interval` | SSO Cookie 轮询间隔，默认 `0.5` 秒，范围 `0.2-2.0` |
| `sso_retry_count` | 自然跳转失败后推进 grok.com 的次数，默认 `3`，范围 `0-5` |
| `sso_retry_interval` | 两次 SSO 推进之间的间隔，默认 `6` 秒，范围 `3-30` |
| `cpa_auto_add` | 是否 SSO→OAuth 并写入 auth |
| `cpa_auth_dir` | 本地 CPA 目录（`xai-*.json`） |
| `grok2api_auth_dir` | 最新 Go 版 Grok2API 可导入的 `grok_build` OAuth JSON 目录 |
| `cpa_remote_url` / `cpa_management_key` | 远程 CPA Management API（可选） |
| `push_token` | 任务完成推送的 Bearer token（可选；也可用环境变量 `PUSH_TOKEN`） |
...
| `MONITOR_STOP_TASKS_ON_EXIT` | `1` | 关闭面板（Ctrl+C、SIGTERM、终端 SIGHUP）时同时停止注册任务；设为 `0` 才允许任务脱离面板继续运行 |
| `PANEL_INCLUDE_TAIL` | 本机 `1`，远程 `0` | 控制面板是否显示已脱敏日志尾部；可显式设为 `0`/`1` |
...
3. 选择 IP 注册数量控制策略，设置 workers / batch 数量与对应阈值 → **启动**
...
| `POST /api/start` · `/api/stop` · `/api/control` · `/api/logs/cleanup` · `/api/blacklist/reset` · `/api/recovery/*` · `/api/proxies/*` · `/api/email-domains/*` | 必须 `Authorization: Bearer <MONITOR_TOKEN>` |
...
- 明细采用服务端分页，每页可选 25 / 50 / 100 / 200 条；支持本页全选、跨页保留选择、批量检测和批量删除
- 一次检测超过 200 条时自动按每批最多 200 条顺序执行，例如 498 条会拆成 `200 + 200 + 98`
- API 和 UI 只返回脱敏端点及 `has_auth`，不会返回代理账号或密码
- worker 只选择“健康 + 启用”的条目，并在每个新账号创建浏览器前热加载一次
- 一个账号从注册、SSO 到 OAuth 始终固定同一代理，不会中途更换出口
- 静态代理网络异常进入短冷却、注册风控进入长冷却；动态 Resin 模板只淘汰当前 `{session}`，不会让整条模板失效
...
macOS 从安装到 Resin、CPA、grok2api 导入的完整说明见
[docs/MAC_LOCAL_START.md](docs/MAC_LOCAL_START.md)。

#### 使用 Resin 订阅出口

本项目不直接解析 VLESS、Trojan、Hysteria2 等订阅节点；应由 Resin 订阅、刷新和
健康检查节点，本项目连接 Resin 提供的 HTTP 正向代理入口。Camoufox/Firefox 不支持
带用户名密码认证的 SOCKS5 代理，因此 Resin 开启强制认证时必须使用 `http://`。
Resin 在远端服务器
仅监听 `127.0.0.1:2260` 时，可在本机先建立隧道：

```bash
ssh -N -L 2260:127.0.0.1:2260 ubuntu@服务器地址
```
...
`{account}` 与 `{session}` 等价。面板会按 worker 和轮换序号生成
`grokreg-wN-rN` 身份；同一轮账号保持同一 Resin 粘性出口，下一轮换身份后仍会
核验公网 IP 确实变化。一条健康模板可供多个 worker 使用，不受“代理条数少于
并发数”的静态代理限制。固定用户名（例如 `Default.macbook`）也能连接，但所有
worker 会共享一个 Resin 身份，不适合多并发轮换注册。

订阅链接和 Resin Token 都属于密钥；不要粘贴到日志、截图或公开 Issue。已经公开
过的订阅链接应在服务端立即轮换。
...
- 多个域名直接在面板文本框中每行填写一个；服务商必须与 `config.json.email_provider` 一致。面板会自动选中当前运行服务商，并在不一致时二次确认
...
任务完成时自动调用 `notify_push.py` 向推送服务发送「任务完成」提醒，
重复发送 3 次、间隔 5 秒，避免单次推送丢失。**推送失败只记日志、不抛异常，
不会影响注册主流程。**

- 编排器 `run_until_100.py`：达到目标 CPA 或全部轮次结束后，推送一次并附带
  CPA 数量 / 目标 / 本轮新增；它启动的批次子进程会自动抑制，避免重复推送
- 单批 `run_batch_headless.py`（面板“单批”模式或命令行直接运行）：批次结束后推送一次

默认请求体：
`{"title":"任务完成","content":"当前任务已完成，请尽快开始新的注册任务。","type":"text"}`
请求头：`Authorization: Bearer <token>`，目标 `https://push.zizheng7zfb.com/api/push`。

token 来源（按优先级）：环境变量 `PUSH_TOKEN` > `config.json` 的 `push_token` 字段。
未配置时跳过推送并记录一条 `[notify] 未配置 push_token…` 日志。也可命令行手动触发：

```bash
.venv/bin/python notify_push.py --title "任务完成" --content "..."
```

可调参数（环境变量）：`PUSH_URL`、`PUSH_RETRIES`（默认 3）、`PUSH_INTERVAL`（默认 5 秒）、
`PUSH_TIMEOUT`（默认 10 秒）。

### 账号汇总与 grok2api

每个注册任务单独一个文件夹 `accounts/tasks/<任务ID>/`（任务ID 形如
`20260807-143000-n40`），存放该任务产生的所有账号文件：

- `accounts/tasks/<任务ID>/<邮箱>.txt`：每账号一条
- `accounts/tasks/<任务ID>/accounts_all.txt`：本任务 `账号----密码----sso` 总表
- `accounts/tasks/<任务ID>/grok2api_web_sso.txt`：本任务每行一个 SSO
- `accounts/tasks/<任务ID>/grok2api_web_accounts.jsonl`：本任务 Grok Web JSONL
- `accounts/tasks/<任务ID>/mail_credentials.txt`、`sso_pending.txt`、`sso_risk_rejected.txt`：本任务附属文件

每次写入以及每轮任务结束时会原子刷新该任务目录内的聚合文件；根目录不再生成
聚合文件（历史遗留账号仍兼容刷新根目录聚合）。另：
- `grok2api_auth/g2a-*.json`：开启 SSO→OAuth 后生成的最新版单账号 Build JSON
- `grok2api_auth/grok2api_build_accounts.jsonl`：自动汇总的 Build 批量导入文件

最新版 grok2api 中，推荐先把 JSONL 导入 **Grok Web**，再执行
**Web → Build 转换**。SSO 注册成功并不等同于已经存在 Grok Build OAuth 凭据。

旧版配置也可在单个 `defaultDomains` 字段中用逗号或空白分隔多个
CloudMail 域名，例如 `"defaultDomains": "mail-a.example.com,mail-b.example.com"`；
不要在 JSON 中重复写两个同名键。只要面板中已配置 CloudMail 域名池，运行时就优先使用面板池。
...
- **扫描全部账号**：递归扫描 `accounts/`（含 `tasks/` 下各任务目录）并对缺失 CPA 的记录执行转换，不删除原账号文件
...
A: 非本机地址默认关闭日志回传。需要时 `export PANEL_INCLUDE_TAIL=1` 后重启 `monitor.py`。
...
- 本机面板默认显示已脱敏日志尾；远程/生产环境建议固定 `PANEL_INCLUDE_TAIL=0`
...
| 绑定安全 | 失败不回退 `0.0.0.0`；日志尾仅在 loopback 默认开启，远程默认关闭 |
