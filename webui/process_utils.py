    ...
    try:
        ...
        if not isinstance(info, dict):
            info = process.as_dict(
                attrs=("pid", "ppid", "cwd", "cmdline", "create_time"),
                ad_value=None,
            )
...
def terminate_managed_processes(
    ...
    *,
    ...
    # Only signal the outermost managed processes first. Supervisors own their
    # browser child groups and can then shut them down without Playwright EPIPE
    # noise caused by terminating parent and child simultaneously.
    root_pids = {
        item["pid"]
        for item in processes
        if item.get("ppid") not in pids
    } or pids
