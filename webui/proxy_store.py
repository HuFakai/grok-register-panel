...
try:
    ...
    from proxy_templates import (
        expand_proxy_template,
        has_proxy_template,
        proxy_template_matches,
        validate_proxy_template_parts,
    )
...
def normalize_proxy(value: object) -> str:
    ...
    try:
        try:
            validate_proxy_template_parts(raw)
        except ValueError as exc:
            raise ProxyValidationError(str(exc)) from exc
        parsed = urlsplit(raw)
...
def _public_item(item: dict, testing_ids: set[str], now: datetime) -> dict:
    ...
    return {
        ...
        "is_template": has_proxy_template(item["url"]),
        ...
        # 该出口 IP 的历史累计使用次数（所有任务合计，来自 log/ip_usage.json）
        "ip_usage": _get_ip_usage(item.get("exit_ip") or ""),
        "source": item.get("source") or "panel",
        "created_at": item.get("created_at") or "",
    }


def _get_ip_usage(exit_ip: str) -> int:
    try:
        from ip_usage_store import get_ip_usage

        return get_ip_usage(exit_ip)
    except Exception:
        return 0
...
def read_proxy_pool(
    *,
    ...
    now = datetime.now(timezone.utc)
    all_items = [_public_item(item, testing_ids, now) for item in state["items"]]
    summary = {
        "total": len(all_items),
        "enabled": sum(1 for item in all_items if item["enabled"]),
        "healthy": sum(1 for item in all_items if item["stored_status"] == "healthy"),
        "unhealthy": sum(1 for item in all_items if item["stored_status"] == "unhealthy"),
        "cooldown": sum(1 for item in all_items if item["stored_status"] == "cooldown"),
        "unknown": sum(1 for item in all_items if item["stored_status"] == "unknown"),
        "usable": sum(
            1
            for item in all_items
            if item["enabled"] and item["stored_status"] == "healthy"
        ),
        "templates": sum(1 for item in all_items if item["is_template"]),
        "usable_templates": sum(
            1
            for item in all_items
            if item["is_template"]
            and item["enabled"]
            and item["stored_status"] == "healthy"
        ),
    }
    pagination = None
    items = all_items
    if page is not None or page_size is not None:
        try:
            normalized_size = max(10, min(200, int(page_size or 50)))
        except (TypeError, ValueError):
            normalized_size = 50
        total_pages = max(1, (len(all_items) + normalized_size - 1) // normalized_size)
        try:
            normalized_page = max(1, min(total_pages, int(page or 1)))
        except (TypeError, ValueError):
            normalized_page = 1
        start = (normalized_page - 1) * normalized_size
        items = all_items[start : start + normalized_size]
        pagination = {
...
def delete_proxies(proxy_ids: object) -> dict:
    requested = {
        str(value or "").strip()
        for value in (
            proxy_ids if isinstance(proxy_ids, (list, tuple, set)) else []
        )
        if re.fullmatch(r"[a-f0-9]{20}", str(value or "").strip())
    }
    if not requested:
        return {"ok": False, "error": "没有选择可删除的代理", "deleted_ids": []}
    with exclusive_file_lock(LOCK_PATH):
        state, _ = _read_unlocked()
        existing_ids = {item["id"] for item in state["items"]}
        deleted_ids = sorted(requested & existing_ids)
        if not deleted_ids:
            return {"ok": False, "error": "所选代理不存在", "deleted_ids": []}
        deleted_set = set(deleted_ids)
        state["items"] = [
            item for item in state["items"] if item["id"] not in deleted_set
        ]
        _write_unlocked(state)
    return {
        "ok": True,
        ...
        "summary": read_proxy_pool()["summary"],
    }
...
def mark_proxy_used(url: object) -> bool:
    ...
    with exclusive_file_lock(LOCK_PATH):
        state, _ = _read_unlocked()
        for item in state["items"]:
            if proxy_template_matches(item["url"], normalized):
...
def record_proxy_result(url: object, outcome: str, error: object = "") -> bool:
    ...
    with exclusive_file_lock(LOCK_PATH):
        state, _ = _read_unlocked()
        for item in state["items"]:
            if not proxy_template_matches(item["url"], normalized):
                continue
            changed = True
            item["last_used_at"] = _utc_now()
            if outcome == "success":
                ...
                item["cooldown_until"] = ""
                item["cooldown_reason"] = ""
            elif has_proxy_template(item["url"]):
                # A Resin template can mint a fresh sticky identity. One failed
                # expanded session must not cool down the whole source template,
                # otherwise a one-entry managed pool becomes unusable and cannot
...
def probe_proxy(url: object, timeout: float = DEFAULT_TEST_TIMEOUT) -> dict:
    ...
    normalized = normalize_proxy(url)
    normalized = expand_proxy_template(
        normalized,
        identity=f"probe-{int(time.time())}",
    )
...
def _run_test_job(job_id: str, selected: list[tuple[str, str]], timeout: float) -> None:
    try:
        batches = [
            selected[offset : offset + MAX_TEST_ITEMS]
            for offset in range(0, len(selected), MAX_TEST_ITEMS)
        ]
        for batch_index, batch in enumerate(batches, 1):
            with _TEST_LOCK:
                if _TEST_JOB.get("job_id") != job_id:
                    return
                _TEST_JOB["batch_index"] = batch_index
                _TEST_JOB["batch_total"] = len(batches)
                _TEST_JOB["testing_ids"] = [proxy_id for proxy_id, _ in batch]
            workers = min(4, max(1, len(batch)))
            with ThreadPoolExecutor(
                max_workers=workers,
                thread_name_prefix="proxy-test",
            ) as executor:
                futures = [
                    executor.submit(_probe_task, proxy_id, url, timeout)
                    for proxy_id, url in batch
                ]
                for future in as_completed(futures):
                    ...
                    with _TEST_LOCK:
                        ...
                        _TEST_JOB["testing_ids"] = [
                            value
                            for value in _TEST_JOB["testing_ids"]
                            if value != proxy_id
                        ]
    finally:
        with _TEST_LOCK:
...
def start_proxy_tests(ids: object = None, *, timeout: float = DEFAULT_TEST_TIMEOUT) -> dict:
    ...
    with _TEST_LOCK:
        ...
        batch_total = max(1, (len(selected) + MAX_TEST_ITEMS - 1) // MAX_TEST_ITEMS)
        _TEST_JOB.update(
            {
                "running": True,
                "job_id": job_id,
                ...
                "finished_at": None,
                "testing_ids": [
                    proxy_id for proxy_id, _ in selected[:MAX_TEST_ITEMS]
                ],
                "batch_index": 1,
