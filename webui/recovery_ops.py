...
def _scoped_files(root_file: Path) -> list[Path]:
    """根目录遗留 + 各任务目录下的同名附属文件（sso_pending 等）。

    账号按任务分目录后，每任务的附属文件在 accounts/tasks/<任务ID>/ 下，
    恢复流程需要汇总所有任务的文件。
    """
    files = []
    if root_file.is_file():
        files.append(root_file)
    tasks_root = ACCOUNTS_DIR / "tasks"
    if tasks_root.is_dir():
        files.extend(
            sorted(
                p for p in tasks_root.rglob(root_file.name) if p.is_file()
            )
        )
    return files
...
def _pending_files() -> list[Path]:
    return _scoped_files(PENDING_FILE)


def _risk_files() -> list[Path]:
    return _scoped_files(RISK_FILE)
...
def _account_records() -> dict[str, str]:
    ...
    for path in sorted(ACCOUNTS_DIR.rglob("*.txt")):
        if path.name in (
...
def recovery_status() -> dict:
    pending = sum(_nonempty_line_count(path) for path in _pending_files())
    ...
    return {
        ...
        "risk_rejected_count": sum(
            _nonempty_line_count(path) for path in _risk_files()
        ),
