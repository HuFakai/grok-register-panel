...
def test_proxy_pool_panel_structure():
    ...
    assert 'id="proxy-select-page"' in html
    assert 'id="proxy-test-selected"' in html
    assert 'id="proxy-delete-selected"' in html
    assert 'id="proxy-page-size"' in html
    assert 'id="proxy-page-prev"' in html
    assert 'id="proxy-page-next"' in html
    ...
    assert 'function testSelectedProxies(' in mon
    assert 'function deleteSelectedProxies(' in mon
    assert 'function toggleProxyPageSelection(' in mon
    ...
    assert '/api/proxies/delete' in mon
...
def test_email_service_and_domain_rotation_panel_structure():
    ...
    assert 'id="domain-active-provider"' in html
    assert "selectDomainProvider" in html
    assert "当前注册实际使用" in html
