...
def test_nested_managed_processes_stop_from_outer_supervisor():
    with tempfile.TemporaryDirectory() as temp:
        root = Path(temp)
        child_script = root / "managed_child.py"
        parent_script = root / "managed_parent.py"
        child_script.write_text("import time\ntime.sleep(30)\n", encoding="utf-8")
        parent_script.write_text(
            "\n".join(
                (
                    "import os, signal, subprocess, sys, time",
                    f"child = subprocess.Popen([sys.executable, {str(child_script)!r}], cwd={str(root)!r}, start_new_session=True)",
        ...
        try:
            ...
            for _ in range(40):
                found = process_utils.find_managed_processes(
                    root,
                    (parent_script.name, child_script.name),
                )
                if {item["pid"] for item in found} == {parent.pid, child_pid}:
                    break
                time.sleep(0.05)
            killed = process_utils.terminate_managed_processes(
                root,
                (parent_script.name, child_script.name),
                grace_seconds=4,
            )
            assert set(killed) == {parent.pid, child_pid}
            assert parent.wait(timeout=5) == 0
            assert not process_utils.find_managed_processes(
                root,
                (parent_script.name, child_script.name),
            )
        finally:
...
if __name__ == "__main__":
    ...
    test_nested_managed_processes_stop_from_outer_supervisor()
