...
from proxy_templates import expand_proxy_template
...
def accounts_side_file(name, task_subdir=None):
    """accounts/ 下的附属文件路径（mail_credentials / sso_pending 等）。

    任务运行期间写入 accounts/tasks/<任务ID>/ 下（任务自包含）；无任务
    上下文时写入根目录（兼容旧调用）。
    """
    base = ensure_accounts_dir()
    sub = task_subdir if task_subdir is not None else current_task_subdir()
    if sub:
        return os.path.join(base, TASKS_SUBDIR, sub, name)
    return os.path.join(base, name)


# 当前注册任务子目录（accounts/tasks/<任务ID>/），供 add_sso_to_cpa 等
# 无显式参数路径读取；任务开始设置、任务收尾清空。
_current_task_subdir: str | None = None


def set_current_task_subdir(subdir):
    global _current_task_subdir
    _current_task_subdir = subdir


def current_task_subdir():
    return _current_task_subdir


def new_task_subdir(count=None, now=None):
    """当前注册任务子目录名：YYYYMMDD-HHMMSS[-n<count>]。

    账号文件写入 accounts/tasks/<任务子目录>/ 下，避免根目录文件过多。
    """
    ts = (now or datetime.datetime.now()).strftime("%Y%m%d-%H%M%S")
    if count:
        return f"{ts}-n{int(count)}"
    return ts


def finalize_account_exports(log_callback=None):
    """Rebuild the per-task aggregate files after each round."""
    result = consolidate_account_exports(ensure_accounts_dir())
    if log_callback:
        log_callback(
            f"[*] 账号汇总已更新: {result['count']} 个账号，"
            f"{result['tasks']} 个任务目录 → 各任务内 "
            "accounts_all.txt / grok2api_web_sso.txt / "
            "grok2api_web_accounts.jsonl"
        )
    return result
...
DEFAULT_CONFIG = {
    ...
    # 面板代理池无健康且可用代理时回退本地网络（用 config.proxy，留空则直连）
    ...
    "cpa_remote_url": "",
    "cpa_management_key": "",
    # 最新 Go 版 Grok2API 的 grok_build JSON 导入目录（默认 grok2api_auth/）
    ...
    # 每个真实出口最多完成的账号槽位；0=不限（只按连续失败换 IP）。
    # 成功和最终失败都计数，内部重试不计数。
    "accounts_per_ip": 2,
    # 同一出口连续最终失败多少个账号才换 IP（始终生效）。
    "ip_failure_rotate_threshold": 3,
    # 单 IP 累计使用次数上限（所有任务累计，见 log/ip_usage.json）；
    # 0=不限。超过后下一轮必须更换 IP。
    "ip_usage_limit": 0,
    # 注册提交后的 SSO 获取策略：快速轮询 + 有界的 grok.com 推进重试。
    "sso_wait_timeout": 50,
...
def classify_failure(exc) -> str:
    ...
    if "未收到验证码" in msg or "验证码阶段失败" in msg or ("验证码" in msg and "失败" in msg):
        return FAIL_CODE
    if "Page.goto" in msg or (
        "打开注册页失败" in msg and ("timeout" in low or "超时" in msg)
    ):
        return FAIL_PAGE_LOAD
    if (
        "浏览器" in msg
...
def get_sso_wait_policy() -> dict:
    """Return bounded SSO waiting options accepted by register_flow."""
    def bounded(name, default, minimum, maximum, cast):
        try:
            value = cast(config.get(name, default))
        except (TypeError, ValueError):
            value = default
        return max(minimum, min(value, maximum))

    return {
        "timeout": bounded("sso_wait_timeout", 50, 15, 180, float),
        "poll_interval": bounded("sso_poll_interval", 0.5, 0.2, 2.0, float),
        "retry_count": bounded("sso_retry_count", 3, 0, 5, int),
        "retry_interval": bounded("sso_retry_interval", 6, 3, 30, float),
    }
...
_proxy_worker_assignments: dict[int, str] = {}
...
def pick_proxy_for_worker(worker_id: int, rotate_idx: int = 0) -> str:
    ...
    if not pool:
        if _proxy_pool_source == "managed-empty" and not config.get(
            "proxy_fallback_to_local", False
        ):
            raise RuntimeError("面板代理池没有健康且启用的代理，请先检测或等待冷却结束")
        # 回退本地网络：config.proxy 可用则走本地代理；留空则浏览器直连
        return str(config.get("proxy", "") or "").strip()
    worker_id = max(0, int(worker_id))
    rotate_idx = max(0, int(rotate_idx))
    idx = (worker_id + rotate_idx) % len(pool)
    with _proxy_pool_lock:
        occupied = {
            proxy
            for owner, proxy in _proxy_worker_assignments.items()
            if owner != worker_id
        }
        selected_raw = next(
            (
                pool[(idx + offset) % len(pool)]
                for offset in range(len(pool))
                if expand_proxy_template(
                    pool[(idx + offset) % len(pool)], worker_id, rotate_idx
                ) not in occupied
            ),
            pool[idx],
        )
        selected = expand_proxy_template(selected_raw, worker_id, rotate_idx)
        _proxy_worker_assignments[worker_id] = selected
    try:
        _mark_managed_proxy_used(selected_raw)
    except Exception:
        pass
    return selected


def release_proxy_for_worker(worker_id: int) -> None:
    with _proxy_pool_lock:
        _proxy_worker_assignments.pop(max(0, int(worker_id)), None)


class ConcurrentIpQuota:
    """Process-local IP quota shared by all registration workers.

    limit=0 表示不限制每出口账号数（仅保留"同一出口不并发复用"约束），
    limit>0 时同一出口累计槽位达到 limit 后拒绝新的认领。
    """
    ...
    def claim(self, worker_id: int, exit_ip: str) -> tuple[bool, int, str]:
        worker_id = max(0, int(worker_id))
        exit_ip = str(exit_ip or "").strip()
        if not exit_ip:
            # 本地回退（无可用代理直连）时没有出口 IP 可核算，直接放行
            return True, 0, ""
        with self._lock:
            current = self._owners.get(worker_id)
            if current == exit_ip:
                used = self._counts.get(exit_ip, 0)
            ...
            other_owner = next(
                (
                    owner
                    for owner, owned_ip in self._owners.items()
                    if owner != worker_id and owned_ip == exit_ip
                ),
                None,
            )
            used = self._counts.get(exit_ip, 0)
            if other_owner is not None:
                return False, used, f"已由 W{other_owner + 1} 使用"
            if self.limit > 0 and used >= self.limit:
                return False, used, "全局配额已满"
            self._owners[worker_id] = exit_ip
            return True, used, ""

    def complete(self, worker_id: int, exit_ip: str) -> int:
        worker_id = max(0, int(worker_id))
        exit_ip = str(exit_ip or "").strip()
        if not exit_ip:
            return 0
        with self._lock:
            if self._owners.get(worker_id) != exit_ip:
                return self._counts.get(exit_ip, 0)
            self._counts[exit_ip] = self._counts.get(exit_ip, 0) + 1
            return self._counts[exit_ip]

    def release(self, worker_id: int) -> None:
        with self._lock:
            self._owners.pop(max(0, int(worker_id)), None)

    def used(self, exit_ip: str) -> int:
        with self._lock:
            return self._counts.get(str(exit_ip or "").strip(), 0)


def get_ip_rotation_policy() -> int:
    """Return bounded accounts_per_ip (0..20)；0=不限每 IP 账号数。"""
    try:
        accounts_per_ip = int(config.get("accounts_per_ip", 2) or 2)
    except (TypeError, ValueError):
        accounts_per_ip = 2
    return max(0, min(accounts_per_ip, 20))


def get_ip_rotation_mode() -> int:
    """Return consecutive-failure rotation threshold (1..20)。"""
    try:
        failure_threshold = int(config.get("ip_failure_rotate_threshold", 3) or 3)
    except (TypeError, ValueError):
        failure_threshold = 3
...
def ip_rotation_reason_after_slot(
    *,
    ...
    """Return the account-boundary rotation reason, or an empty string.

    判定顺序：每 IP 账号配额（>0 时）→ 单 IP 累计使用上限（>0 时，跨任务
    累计）→ 连续失败阈值（始终生效）。
    """
    if accounts_per_ip > 0 and used_on_ip >= accounts_per_ip:
        return f"每IP账号配额 {used_on_ip}/{accounts_per_ip}"
    if ip_usage_limit > 0 and ip_usage >= ip_usage_limit:
        return f"IP累计使用 {ip_usage}/{ip_usage_limit}"
    if consecutive_failures >= failure_threshold:
        return f"连续失败 {consecutive_failures}/{failure_threshold}"
    return ""


def proxy_count_for_worker(_worker_id: int) -> int:
    """Number of currently usable endpoints that can be checked during rotation."""
    return max(1, len(load_proxy_pool()))


def rotate_browser_to_new_exit(
    worker_id: int,
    rotate_idx: int,
    previous_ip: str,
    ip_started_at: float,
    *,
    ...
    """Rotate endpoints and refuse to start a new account on an unchanged exit.

    不再按 Sticky 时长等待：只在轮询周期内（90 秒上限）反复换口核验。
    """
    checks_per_pass = min(proxy_count_for_worker(worker_id), 24)
    ...
    while not cancel_callback():
        ...
        try:
            ...
            checked += 1
            record_proxy_boot_failure(proxy, exc)
            log_callback(
                f"[!] [IP策略] 新出口启动失败: "
                f"{redact_sensitive_log_line(str(exc))}"
            )
        else:
            current_ip = get_exit_ip() or ""
            checked += 1
            if current_ip and (not previous_ip or current_ip != previous_ip):
                log_callback(
                    f"[+] [IP策略] 出口核验通过: "
                    f"{previous_ip or '?'} → {current_ip}"
                )
                return rotate_idx, current_ip, time.monotonic()
            log_callback(
                f"[!] [IP策略] 出口仍为 {current_ip or '?'}，不会开始下一轮"
            )

        if checked % checks_per_pass:
            if time.monotonic() >= deadline:
                break
            continue

        if time.monotonic() < deadline:
            log_callback("[*] [IP策略] 10 秒后再次核验出口")
            sleep_with_cancel(10, cancel_callback)

        if time.monotonic() >= deadline:
            break

    if cancel_callback():
        raise RegistrationCancelled("IP 切换核验期间收到停止请求")
    raise RuntimeError(
        f"IP 切换核验失败：出口仍为 {previous_ip or '?'}；"
        "请准备多个不同 Sticky 会话/端口，并检查 DataImpulse 会话时长"
    )
...
def _append_sso_pending(email: str, sso: str, log_callback=None, task_subdir=None):
    """CPA 失败时保留 SSO，便于事后 sso_to_auth_json 重转。

    任务运行期间写入任务目录（任务自包含），否则写根目录。
    """
    try:
        path = accounts_side_file("sso_pending.txt", task_subdir=task_subdir)
        line = f"{email}----{sso}\n" if email else f"{sso}\n"
...
def _append_sso_risk_rejected(email: str, sso: str, details: str, log_callback=None, task_subdir=None):
    """保存注册风控拒绝的 SSO；该类账号不进入待重转队列。"""
    try:
        path = accounts_side_file("sso_risk_rejected.txt", task_subdir=task_subdir)
...
class GrokRegisterGUI:
    ...
    def setup_ui(self):
        ...
        add_label(4, 0, "每 IP 账号数 (0=不限):")
        self.accounts_per_ip_var = tk.StringVar(
            value=str(config.get("accounts_per_ip", 2))
        )
        self.accounts_per_ip_spinbox = tk.Spinbox(
            config_frame,
            from_=0,
            to=20,
            width=8,
            textvariable=self.accounts_per_ip_var,
            bg=UI_ENTRY_BG,
            fg=UI_FG,
            insertbackground=UI_FG,
            buttonbackground=UI_BUTTON_BG,
        ...
        add_field(self.accounts_per_ip_spinbox, 4, 1, sticky=tk.W)
    ...
    def start_registration(self):
        ...
        try:
            # 0=不限每 IP 账号数；>0 时满额换 IP（连续失败换 IP 始终生效）
            config["accounts_per_ip"] = max(
                0, min(20, int(self.accounts_per_ip_var.get()))
            )
        except ValueError:
            config["accounts_per_ip"] = 2
        save_config()
    ...
    def _run_registration_entry(self, count, workers):
        ...
        # 本次任务专属目录：accounts/tasks/<任务ID>/，避免根目录堆满账号文件
        task_subdir = new_task_subdir(count)
        set_current_task_subdir(task_subdir)
        ...
        try:
            ...
            pass
        try:
            # 任务共享派发：worker 每轮从池里领一个槽位，快 worker 会接走
            # 慢 worker 尚未领取的槽位，避免静态分块导致最后只剩 1 个在跑。
            slot_pool = SharedSlotPool(count)
            if workers <= 1:
                self.run_registration(
                    count, worker_id=0, workers=1,
                    slot_pool=slot_pool, task_subdir=task_subdir,
                )
            else:
                self.log(f"[*] 实际并发 worker={workers}，任务共享派发")
                threads = []
                for wid in range(workers):
                    t = threading.Thread(
                        target=self.run_registration,
                        args=(count, wid, workers, slot_pool, task_subdir),
                        daemon=True,
                    )
                    t.start()
                    threads.append(t)
            ...
            set_current_task_subdir(None)
            # 协调线程自身无浏览器；各 worker 线程 finally 已各自 stop
            try:
                finalize_account_exports(log_callback=self.log)
            except Exception as export_exc:
                self.log(
                    f"[!] 账号汇总失败，单账号文件仍保留: "
                    f"{redact_sensitive_log_line(str(export_exc))}"
                )
            self._set_running_ui(False)
            self.log(
    ...
    def run_registration(self, count, worker_id=0, workers=1, slot_pool=None, task_subdir=None):
        ...
        pool = slot_pool or SharedSlotPool(count)
        try:
            try:
            ...
            while True:
                ...
                if not retry_same_slot and pool.remaining() <= 0:
                    continue
                # 账号间随机间隔
...
class SharedSlotPool:
    ...
    原实现按 divmod 把 count 静态切成每 worker 一份，快 worker 跑完自己的
    份额就退出，慢 worker（Turnstile 反复卡、代理换口多）掉队后并发只剩 1。
    改为共享领取：worker 每轮从池里领一个槽位，领完即止，快 worker 会继续
    接走慢 worker 尚未领取的槽位，全程保持并发。
    """
    ...
    def fail_unclaimed(self):
        """worker 因基础设施故障（代理耗尽/换口失败）提前退出时，把尚未
        领取的槽位一次性全部标记为失败，返回数量。避免各 worker 各记各的
        导致重复计数，或进度永远到不了 total 触发 supervisor 反复重启。"""
        with self._lock:
            if self._next >= self.total:
                return 0
            left = self.total - self._next
...
def run_registration_cli(count):
    ...
    accounts_per_ip = get_ip_rotation_policy()
    failure_rotate_threshold = get_ip_rotation_mode()
    ip_usage_limit = get_ip_usage_limit()
    with _proxy_pool_lock:
        _proxy_worker_assignments.clear()
    ip_quota = ConcurrentIpQuota(accounts_per_ip)
    if accounts_per_ip > 0:
        cli_log(
            f"[*] [IP策略] 每IP账号数={accounts_per_ip} | 所有 worker 合计每个出口最多 "
            f"{accounts_per_ip} 个账号槽位（成功/最终失败都计数），超过后换 IP | "
            f"连续最终失败 {failure_rotate_threshold} 次后切换 | "
            "同一出口不并发复用 | 切换后强制核验"
        )
    else:
        cli_log(
            f"[*] [IP策略] 每IP账号数=不限 | 仅按连续最终失败 "
            f"{failure_rotate_threshold} 次后切换 | "
            "同一出口不并发复用 | 切换后强制核验"
        )
    if ip_usage_limit > 0:
        cli_log(
            f"[*] [IP策略] 单IP累计使用上限={ip_usage_limit} 次"
            "（所有任务累计，超过后必须换 IP）"
        )
    else:
        cli_log("[*] [IP策略] 单IP累计使用上限=不限")
    ...
    _sso_policy = get_sso_wait_policy()
    cli_log(
        f"[*] SSO等待策略: 总等待 {_sso_policy['timeout']:g}s | "
        f"轮询 {_sso_policy['poll_interval']:g}s | "
        f"推进重试 {_sso_policy['retry_count']} 次 | "
        f"重试间隔 {_sso_policy['retry_interval']:g}s"
    )
    # 启动前清理上次崩溃 / 强杀残留的临时 profile 目录
    try:
    ...
    try:
        ...
        if _conn.has_blocking_xai_failure(startup_checks) and not config.get(
            "proxy_fallback_to_local", False
        ):
    ...
    if workers >= 1:
        # CLI 统一走 worker 路径；单并发也必须执行代理配额与出口核验。
        ...
        def worker(wid):
            ...
            rotate_idx = 0
            active_ip = ""
            ip_started_at = time.monotonic()
            consecutive_failures = 0

            def claim_or_rotate(
                current_rotate_idx: int,
                current_ip: str,
                ...
                while not controller.should_stop():
                    ...
                    cli_log(
                        f"[W{wid+1}] [*] [IP策略] 跳过出口 {current_ip or '?'}："
                        f"{reason or '不可用'}，全局配额 "
                        f"{used}/{'不限' if accounts_per_ip <= 0 else accounts_per_ip}"
                    )
                ...
                raise RegistrationCancelled("IP 配额核验期间收到停止请求")

            try:
                try:
                    px = pick_proxy_for_worker(wid, rotate_idx)
                except Exception as proxy_exc:
                    left = slot_pool.fail_unclaimed()
                    local_fail = left
                    local_fail_stats[FAIL_BROWSER] = local_fail_stats.get(FAIL_BROWSER, 0) + left
                    cli_log(
                ...
                try:
                    ...
                    if not booted:
                        ...
                        local_fail = left
                        local_fail_stats[FAIL_BROWSER] = local_fail_stats.get(FAIL_BROWSER, 0) + left
                        mark_slot_completed(left)
                        cli_log(
                            f"[W{wid+1}] [-] 浏览器启动失败，{left} 个剩余任务记为失败: "
                        ...
                        return
                active_ip = get_exit_ip() or ""
                ip_started_at = time.monotonic()
                rotate_idx, active_ip, ip_started_at, used_on_ip = claim_or_rotate(
                    rotate_idx,
                    active_ip,
                    ip_started_at,
                )
                cli_log(
                    f"[W{wid+1}] [*] [IP策略] 本轮出口={active_ip or '?'}，"
                    f"全局配额 {used_on_ip}/"
                    f"{'不限' if accounts_per_ip <= 0 else accounts_per_ip}"
                )
                i = 0
                retry = 0
                worker_stop = False
                retry_same_slot = False
                while not controller.should_stop() and not worker_stop:
                    if retry_same_slot:
                        # 同槽位换口重试：不领取新任务，复用当前槽位
                        retry_same_slot = False
                    else:
                        claimed = slot_pool.claim()
                        if claimed is None:
                            break
                        i = claimed
                    slot_start_i = i
                    try:
                        ...
                        mark_slot_completed()
                        if kind == FAIL_PAGE_LOAD:
                            # 页面加载超时：计作失败并立即换 IP，不等待连续失败阈值
                            rotate_idx += 1
                            cli_log(f"[W{wid+1}] [*] [IP策略] 页面加载超时，切换 sticky #{rotate_idx}")
                        elif kind == FAIL_RISK and accounts_per_ip > 0:
                            rotate_idx += 1
                            cli_log(f"[W{wid+1}] [*] 风控拒绝，切换 sticky #{rotate_idx}")
                        elif accounts_per_ip > 0 and (
                            blank_ui or proxy_dead or turnstile_stuck or profile_soft or kind in (
                            FAIL_TURNSTILE,
                            FAIL_PROFILE,
                            )
                        ):
                        ...
                        if (retry_same_slot or slot_pool.remaining() > 0) and not controller.should_stop():
                            slot_completed = i > slot_start_i
                            if slot_completed:
                                # 槽位完成（成功或最终失败）即累计该出口 IP 使用次数，
                                # 跨任务持久化到 log/ip_usage.json；内部重试不计。
                                if active_ip:
                                    record_ip_usage(active_ip)
                                if accounts_per_ip > 0:
                                    used_on_ip = ip_quota.complete(wid, active_ip)
                                    cli_log(
                                        f"[W{wid+1}] [*] [IP策略] 出口 "
                                        f"{active_ip or '?'} 全局已完成 {used_on_ip}/"
                                        f"{accounts_per_ip} 个账号槽位"
                                    )
                                else:
                                    cli_log(
                                        f"[W{wid+1}] [*] [IP策略] 出口 "
                                        f"{active_ip or '?'} 连续失败 "
                                        f"{consecutive_failures}/{failure_rotate_threshold}"
                                    )
                            reason = ""
                            if slot_completed:
                            ...
                            if reason:
                                cli_log(
                                    f"[W{wid+1}] [*] [IP策略] {reason} 已到，"
                                    "下一轮前切换并核验出口"
                                )
                                try:
                                    ip_quota.release(wid)
                                    rotate_idx, active_ip, ip_started_at = (
                                        rotate_browser_to_new_exit(
                                            wid,
                                            rotate_idx,
                                            active_ip,
                                            ip_started_at,
                                            log_callback=lambda m: cli_log(
                                                f"[W{wid+1}] {m}"
                                            ),
                                            cancel_callback=controller.should_stop,
                                        )
                                    ...
                                    consecutive_failures = 0
                                    sleep_with_cancel(0.5, controller.should_stop)
                                except RegistrationCancelled:
                                    worker_stop = True
                                    ...
                                    local_fail_stats[FAIL_BROWSER] = (
                                        local_fail_stats.get(FAIL_BROWSER, 0)
                                        + left
                                    )
                                    mark_slot_completed(left)
                                    cli_log(
                                        f"[W{wid+1}] [-] [IP策略] "
                                        f"{redact_sensitive_log_line(str(rotate_exc))}；"
                                        f"为避免复用旧 IP，本 worker 停止，"
                                        f"剩余 {left} 个未领取任务记为失败"
                                    )
                                    worker_stop = True
                                continue
                            try:
                            ...
                            try:
                                ...
                                start_browser(log_callback=lambda m: cli_log(f"[W{wid+1}] {m}"))
                                observed_ip = get_exit_ip() or ""
                                previous_ip = active_ip
                                if observed_ip and observed_ip != previous_ip:
                                    cli_log(
                                        f"[W{wid+1}] [*] [IP策略] 提前换口已生效: "
                                        f"{previous_ip or '?'} → {observed_ip}"
                                    )
                                    active_ip = observed_ip
                                    ip_started_at = time.monotonic()
                                (
                                    rotate_idx,
                                    active_ip,
                                ...
                                if "面板代理池没有健康且启用的代理" in str(boot_exc):
                                    ...
                                    local_fail += left
                                    local_fail_stats[FAIL_BROWSER] = (
                                        local_fail_stats.get(FAIL_BROWSER, 0) + left
                                    )
                                    mark_slot_completed(left)
                                    cli_log(
    ...
    try:
        ...
        while i < count:
            ...
            try:
                ...
                if (
                    kind == FAIL_RISK
                    or proxy_dead
                    or kind in (FAIL_TURNSTILE, FAIL_PROFILE, FAIL_PAGE_LOAD)
                ):
                    single_rotate_idx += 1
                cli_log(
                    f"[-] 注册失败 [{FAIL_LABELS.get(kind, kind)}]: "
        ...
        try:
            ...
            pass
        try:
            finalize_account_exports(log_callback=cli_log)
