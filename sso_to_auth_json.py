...
写出：
  ...
  - 最新 Go 版 Grok2API 可导入的 grok_build JSON
  - 官方 ~/.grok 风格 issuer::client_id 嵌套 auth
...
from account_exports import GENERATED_EXPORT_FILENAMES
...
def grok2api_auth_filename(entry: dict, email: str = "") -> str:
    """Latest grok2api Build import filename."""
    ident = (
        str(email or "").strip()
...
def token_to_grok2api_build_entry(token: dict, email: str = "") -> dict:
    """Convert OAuth tokens to chenyme/grok2api's current Build import shape."""
    access = str(token.get("access_token") or token.get("key") or "").strip()
    refresh = str(token.get("refresh_token") or "").strip()
    if not access and not refresh:
        raise ValueError("Grok2API Build 凭据缺少 access_token / refresh_token")
    id_token = str(token.get("id_token") or "").strip()
    claims = decode_jwt_payload(id_token or access)
    access_claims = decode_jwt_payload(access)
    user_id = str(
        claims.get("sub")
        or claims.get("principal_id")
        or access_claims.get("sub")
        or access_claims.get("principal_id")
        or ""
    )
    resolved_email = str(
        email or claims.get("email") or access_claims.get("email") or ""
    ).strip()
    expires_at = ""
    exp = access_claims.get("exp") or claims.get("exp")
    if exp:
        expires_at = rfc3339_ns(float(exp))
    ...
    return {
        ...
        "client_id": str(token.get("client_id") or CLIENT_ID),
        "access_token": access,
        "refresh_token": refresh,
        "id_token": id_token,
        ...
        "team_id": str(claims.get("team_id") or access_claims.get("team_id") or ""),
    }


def write_grok2api_auth(auth_dir: Path, token: dict, email: str = "") -> Path:
    """Write a JSON file accepted by current grok2api's Grok Build importer."""
    ensure_private_dir(auth_dir)
    entry = token_to_grok2api_build_entry(token, email=email)
    path = auth_dir / grok2api_auth_filename(entry, email=email)
    atomic_write_json(path, entry)
    _refresh_grok2api_build_export(auth_dir)
    return path


def _refresh_grok2api_build_export(auth_dir: Path) -> Path:
    """Create one JSONL file for bulk import into current grok2api Build."""
    target = auth_dir / "grok2api_build_accounts.jsonl"
    with exclusive_file_lock(auth_dir / ".grok2api_build_export.lock"):
        entries: dict[str, dict] = {}
        for candidate in sorted(auth_dir.glob("g2a-*.json")):
            try:
                value = json.loads(candidate.read_text(encoding="utf-8"))
            except (OSError, ValueError, TypeError, json.JSONDecodeError):
                continue
            if not isinstance(value, dict) or value.get("provider") != "grok_build":
                continue
            identity = str(
                value.get("user_id")
                or value.get("email")
        ...
        text = "".join(
            ...
            for _identity, value in sorted(entries.items())
        )
...
def load_sso_records(
    ...
    if accounts_dir:
        ...
        if account_root.is_dir():
            # 递归扫描：根目录历史账号 + tasks/<任务>/ 下各任务账号
            for candidate in sorted(account_root.rglob("*.txt")):
                if pending_only and candidate.name != "sso_pending.txt":
                    continue
                if candidate.name in (
                    {"mail_credentials.txt", "sso_risk_rejected.txt"}
...
def _pending_queue_files(records: list[SsoInput], explicit_sso: str | None) -> set[Path]:
    """返回需要原子消费出队的队列文件集合。

    --sso 模式为单文件；--accounts-dir --pending-only 模式下为各任务目录
    （及根目录遗留）的 sso_pending.txt。
    """
    if explicit_sso:
        return {Path(explicit_sso)}
    return {
        Path(record.source)
        for record in records
        if record.source and Path(record.source).name == "sso_pending.txt"
    }
...
def main() -> int:
    ...
    ap.add_argument("--consume-success", action="store_true", help="成功后从 SSO 队列文件原子移除对应记录（--sso 文件，或 --accounts-dir 扫描到的各任务 sso_pending.txt）")
    ap.add_argument("--pending-only", action="store_true", help="配合 --accounts-dir：只处理各任务目录的 sso_pending.txt")
    ...
    if args.consume_success and not (args.sso or (args.accounts_dir and args.pending_only)):
        ap.error("--consume-success 只能与 --sso FILE 或 --accounts-dir --pending-only 一起使用")
    ...
    consume_targets = _pending_queue_files(records, args.sso)
    ...
    if already_present:
        ...
        if args.consume_success:
            for target in consume_targets:
                consume_successful_records(target, existing_ssos)
    ...
    for i, record in enumerate(records, 1):
        ...
        try:
            ...
            if args.consume_success:
                for target in consume_targets:
                    consume_successful_records(target, {sso})
    ...
    if args.consume_success:
        remaining = sum(
            consume_successful_records(target, succeeded_ssos)
            for target in consume_targets
        )
