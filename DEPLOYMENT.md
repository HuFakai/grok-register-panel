...
冷却时会停止对应任务；配置 `proxy_fallback_to_local: true` 时改为回退
`config.proxy`（留空则浏览器直连）继续建号。一个账号开始后，注册、SSO 与
OAuth 全程固定同一出口。
远端 Resin 仅绑定 `127.0.0.1:2260` 时，先用 SSH `-L 2260:127.0.0.1:2260`
转发到本机，再导入
`socks5h://Default.{session}:TOKEN@127.0.0.1:2260`。原始订阅链接不能直接
导入本项目；`{session}`（或 `{account}`）会在每个 worker、每轮切换时展开为
独立粘性身份。
